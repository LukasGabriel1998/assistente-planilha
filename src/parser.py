...
def _extract_sale_id_from_text(text: str) -> Optional[str]:
    patterns = [
        r"\bid\s*de\s*cliente\s*(?:numero|n[uú]mero|n\.?|codigo|c[oó]digo)?\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
        r"\bid\s*de\s*venda\s*(?:numero|n[uú]mero|n\.?|codigo|c[oó]digo)?\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
        r"\bid\s*cliente\s*(?:numero|n[uú]mero|n\.?|codigo|c[oó]digo)?\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
        r"\bid\s*venda\s*(?:numero|n[uú]mero|n\.?|codigo|c[oó]digo)?\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
        r"\bc[oó]digo\s*(?:da\s*venda)?\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
        r"\bcliente\s+com\s+id\s*([a-zA-Z0-9_-]{1,30})",
        r"\bcliente\s+([a-zA-Z0-9_-]{1,30})\b",
        r"\bid\s*[:\-]?\s*([a-zA-Z0-9_-]{1,30})",
    ]
    for pattern in patterns:
        match = re.search(pattern, text, flags=re.IGNORECASE)
        if match:
...
def _extract_material_allocations(text: str, reference_date: date) -> list[MaterialAllocation]:
    ...
    id_pattern = r"(?:cliente|id\s*(?:de\s*)?(?:venda|cliente))\s*([a-zA-Z0-9_-]{1,30})"
    ...
    explicit_patterns = [
        ...
        (
            re.compile(
                r"(?:adiciona|adicionar|coloca|colocar|lanca|lança)\s+"
                + loose_id_pattern
                + r"(?:\s*,\s*|\s+)"
                + money_pattern
                + r"(?:\s+\w+){0,4}?\s*(?:de\s+material|material)?",
...
def parse_financial_message(message: str, reference_date: Optional[date] = None) -> ParseResult:
    ...
    material_reference_only = bool(
        ...
        and (
            ...
            or (sale_id and _extract_amount_after_prefix(
                raw_text,
                prefixes=[
                    r"material",
                    r"materia prima",
    ...
    warnings: list[str] = []
    missing: list[str] = []
    material_only_by_sale_id = bool((material_allocations or (sale_id and material_cost)) and not total_value)
    # Só exige cliente e valor; produto é opcional (usa "Serviço para {cliente}" se faltar)
    if not material_only_by_sale_id:
        if not customer:
    ...
    elif material_only_by_sale_id and sale_id:
        description = f"Material da venda {sale_id}"
    elif material_only_by_sale_id and material_allocations:
